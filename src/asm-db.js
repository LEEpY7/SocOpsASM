...
;(function cleanupLegacySeedAssets() {
  ...
  const tx = asmDb.transaction(() => {
    sampleIps.forEach(ip => {
      asmDb.prepare('DELETE FROM asset_snapshot WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM vuln_snapshot WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM asset_change_log WHERE asset_ip=?').run(ip)
      asmDb.prepare('DELETE FROM asset_current WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM service_current WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM http_current WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM vuln_current WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM vulnerability_finding WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM http_endpoint WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM network_service WHERE ip=?').run(ip)
      asmDb.prepare('DELETE FROM dns_record WHERE fqdn LIKE ? OR fqdn LIKE ?').run(`%${ip}%`, `sample-%${ip}%`)
      asmDb.prepare('DELETE FROM asset WHERE ip=?').run(ip)
    })

    asmDb.prepare(`
      DELETE FROM asset_name
      WHERE fqdn IN (
        'hanwhalife.com','www.hanwhalife.com','mobile.hanwhalife.com','m.hanwhalife.com',
        'direct.hanwhalife.com','api.hanwhalife.com','admin.hanwhalife.com','dev.hanwhalife.com',
        'company.hanwhalife.com','recruit.hanwhalife.com','cdn.hanwhalife.com','static.hanwhalife.com',
        'intranet.hanwhalife.internal','db01.hanwhalife.internal'
      )
    `).run()
  })
