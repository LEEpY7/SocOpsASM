...
[PM2]
  ...
  └─ finmonitor  (port 3000)
        ├─ 가용성 모니터링  → better-sqlite3 (finmonitor.db, 7일 보관)
        └─ ASM 파이프라인  → better-sqlite3 (asm.db)
...
│   ├── db.js                      # 가용성 모니터링 DB (finmonitor.db)
...
│   ├── asm-db.js                  # ASM DB 스키마 및 시드 데이터 (asm.db)
